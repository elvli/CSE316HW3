...
export const useGlobalStore = () => {
    ...
    const storeReducer = (action) => {
        ...
        switch (type) {
            ...
            case GlobalStoreActionType.SONG_FOR_DELETION: {
                return setStore({
                    idNamePairs: store.idNamePairs,
                    currentList: null,
                    newListCounter: store.newListCounter,
    ...
    store.deletePlaylist = function (modal) {
        if(store.markedList){
            store.closeModal(modal);
            async function asyncDeletePlaylist(){
                let id = store.markedList._id;
    ...
    store.getMarkedListName = function () {
        if (store.markedList){
            return store.markedList.name;
        }
    }

    store.markSongForDeletion = function (id, index) {
        storeReducer({
    ...
    store.getMarkedSongName = function () {
        // if (store.markedList){
        //     return store.markedList.name;
        // }
    }
    ...
    //         async function updateList(playlist) {
    //             response = await api.DeleteSong(playlist._id);
    //             if (response.data.success) {
    //                 async function getListPairs(playlist) {
    //                     response = await api.getPlaylistPairs();
    //                     if (response.data.success) {
